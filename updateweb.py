# updateweb.py  (versão para Streamlit web com cache)
import re
import pandas as pd
...
def _read_sheet_cached(sheet_name: str) -> pd.DataFrame:
    """Faz download e lê uma aba do Google Sheets (cacheado por 5 minutos)."""
    return pd.read_excel(URL_PLANILHA, sheet_name=sheet_name)

def _safe_read(sheet_name: str, expected_cols=None) -> pd.DataFrame:
    """Lê usando cache; se falhar, retorna DF vazio com as colunas esperadas."""
    try:
        df = _read_sheet_cached(sheet_name)
        if expected_cols:
...
def processar_planilha_com_resgates() -> pd.DataFrame:
    try:
        ...
        # --- TAB04: Lançamentos extras por DOC -> mapear para Nome via TAB03 ---
        ...
        df_agr["Total Vendido (R$)"] = df_agr["Total Vendido (R$)"].fillna(0.0) + df_agr["Valor Extra (R$)"].fillna(0.0)
        ...
        # --- TAB02: Resgates em R$ -> converter p/ pontos (floor) ---
        ...
        # --- Juntar CPF/CNPJ normalizado via TAB03 ---
        df_m = df_m.merge(
...
def carregar_resgates_por_nome(nome: str) -> pd.DataFrame:
    ...
    if df2.empty:
        return pd.DataFrame(columns=["DataHora Resgate", "Pontos Resgatados", "Valor Resgatado", "Usuario"])
    ...
    cols = ["DataHora Resgate", "Pontos Resgatados", "Valor Resgatado", "Usuario"]
    for c in cols:
